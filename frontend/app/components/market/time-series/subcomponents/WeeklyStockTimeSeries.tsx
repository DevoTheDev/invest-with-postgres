...
import { WeeklyTimeSeriesResponse, WeeklyMetaData, WeeklyTimeSeries, WeeklyOHLCV, defaultWeeklyResponse } from '@/app/defaults/alphaVantage-defaults/defaultWeeklyDataWithSymbol';
import Pagination from '../../../sections/Pagination';

interface WeeklyStockTimeSeriesProps {
  data?: WeeklyTimeSeriesResponse;
}
...
const WeeklyStockTimeSeries: React.FC<WeeklyStockTimeSeriesProps> = ({ data = defaultWeeklyResponse }) => {
  const metaData: WeeklyMetaData = data['Meta Data'];
  const timeSeries: WeeklyTimeSeries = data['Weekly Time Series'];

  // Convert time series object to array and sort by date (most recent first)
  const sortedWeeks = Object.entries(timeSeries).sort(([dateA], [dateB]) =>
  ...
  const currentData = sortedWeeks.slice(startIndex, endIndex);

  return (
    <div className="bg-white shadow-lg rounded-lg p-6 max-w-4xl mx-auto">
      <h2 className="text-2xl font-bold mb-4 text-gray-800">
        {metaData['2. Symbol']} Weekly Stock Data
      </h2>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm text-left text-gray-600">
          ...
          <tbody>
            {currentData.map(([date, ohlcv]: [string, WeeklyOHLCV]) => (
...
export default WeeklyStockTimeSeries;
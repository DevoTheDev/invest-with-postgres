...
import { MonthlyTimeSeriesResponse, MonthlyMetaData, MonthlyOHLCV, MonthlyTimeSeries, defaultMonthlyResponse } from '@/app/defaults/alphaVantage-defaults/defaultMonthlyDataWithSymbol';
import Pagination from '../../../sections/Pagination';


interface MonthlyStockTimeSeriesProps {
  data?: MonthlyTimeSeriesResponse;
}
...
const MonthlyStockTimeSeries: React.FC<MonthlyStockTimeSeriesProps> = ({ data = defaultMonthlyResponse }) => {
  const metaData: MonthlyMetaData = data['Meta Data'];
  const timeSeries: MonthlyTimeSeries = data['Monthly Time Series'];

  // Convert time series object to array and sort by date (most recent first)
  const sortedMonths = Object.entries(timeSeries).sort(([dateA], [dateB]) =>
  ...
  const currentData = sortedMonths.slice(startIndex, endIndex);

  return (
    <div className="bg-white shadow-lg rounded-lg p-6 max-w-5xl mx-auto">
      <h2 className="text-2xl font-bold mb-4 text-gray-800">
        {metaData['2. Symbol']} Monthly Stock Data
      </h2>
      <div className="mb-4 space-y-2">
        ...
        <p className="text-sm text-gray-600">
          <span className="font-semibold">Time Zone:</span> {metaData['4. Time Zone']}
        </p>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full text-sm text-left text-gray-600">
          ...
          <tbody>
            {currentData.map(([date, ohlcv]: [string, MonthlyOHLCV]) => (
...
export default MonthlyStockTimeSeries;
...
const NeatStringify_1 = require("./NeatStringify");
const colors = {
    reset: '\x1b[0m',
...
const solidBorder = (length, color, isTop = false) => {
    const chars = isTop ? '═╦═' : '═╩═';
    return color + chars.repeat(Math.floor(length / 3) + 1).slice(0, length) + colors.reset;
};
// Internal separator for spacing
const internalSeparator = (length, color) => color + `╟${'─'.repeat(length - 2)}╢` + colors.reset;
// Get caller info
const getCallerInfo = () => {
    var _a;
    try {
...
const logMessage = (type, message, obj, options = {}) => {
    ...
    const formattedObj = obj
        ? (0, NeatStringify_1.NeatStringify)(obj)
            .split('\n')
            .map((line) => `║  ${padEnd(truncate(line, innerWidth - 2), innerWidth - 2)}  ║`)
            .join('\n')
        : '';
    // Format context
